...
const validationFilters = document.querySelectorAll("[data-validation-filter]:not([data-validation-server-filter])");
const validationServerFilters = document.querySelectorAll("[data-validation-server-filter]");
const validationFilterForm = document.querySelector("#validation-filter-form");
...
const validationTeammatesBuilder = document.querySelector("[data-validation-teammates-builder]");
const validationPlayerSelect = document.querySelector("#validation-player");
let pendingValidationStatusRows = [];
let pendingValidationStatus = "";
let validationFilterSubmitTimer = null;

if (validationFilterForm && validationServerFilters.length) {
    const submitValidationFilters = () => {
        const params = new URLSearchParams();

        validationServerFilters.forEach((filter) => {
        ...
        const nextUrl = query
            ? `${validationFilterForm.action}?${query}`
            : validationFilterForm.action;
    ...
    validationFilterForm.addEventListener("submit", (event) => {
        event.preventDefault();
        submitValidationFilters();
    });

    validationServerFilters.forEach((filter) => {
        const submitFilters = () => {
            window.clearTimeout(validationFilterSubmitTimer);
            validationFilterSubmitTimer = window.setTimeout(() => {
                submitValidationFilters();
            }, filter.type === "search" ? 280 : 0);
        };

        filter.addEventListener(filter.type === "search" ? "input" : "change", submitFilters);
    });
}
...
if (validationTeammatesBuilder) {
    const teammatesList = validationTeammatesBuilder.querySelector("[data-validation-teammates-list]");
    const addButton = validationTeammatesBuilder.querySelector("[data-validation-add-teammate]");

    const syncTeammateOptions = () => {
        const selects = Array.from(validationTeammatesBuilder.querySelectorAll('select[name="teammate_user_id[]"]'));
        const mainPlayerValue = validationPlayerSelect?.value || "";
        selects.forEach((select) => {
            if (mainPlayerValue && select.value === mainPlayerValue) {
                select.value = "";
            }
        });
        ...
        selects.forEach((select) => {
            Array.from(select.options).forEach((option) => {
                const isMainPlayer = option.value !== "" && option.value === mainPlayerValue;
                const isAlreadySelected = option.value !== "" && option.value !== select.value && selectedValues.includes(option.value);

                option.hidden = isMainPlayer || isAlreadySelected;
            });
        });
    };

    validationPlayerSelect?.addEventListener("change", syncTeammateOptions);

    teammatesList?.querySelectorAll('select[name="teammate_user_id[]"]').forEach((select) => {
        select.addEventListener("change", syncTeammateOptions);
    });

    addButton?.addEventListener("click", () => {
        const firstRow = teammatesList?.querySelector(".admin-validation-group__row");

        if (!firstRow || !teammatesList) {
            return;
        }

        const nextRow = firstRow.cloneNode(true);
        const nextSelect = nextRow.querySelector('select[name="teammate_user_id[]"]');
        const nextCharacters = nextRow.querySelector('input[name="teammate_characters[]"]');

        if (nextSelect) {
            nextSelect.value = "";
            nextSelect.addEventListener("change", syncTeammateOptions);
        }

        if (nextCharacters) {
            nextCharacters.value = "1";
        }
...
if (validationRows.length) {
    ...
    const applyValidationFilters = () => {
        ...
        activeFilters.player = activeFilters.player || "all";
        ...
        validationRows.forEach((row) => {
            ...
            if (row.hidden) {
                const checkbox = row.querySelector("[data-validation-select]");
                if (checkbox) {
    ...
    validationBulkApply?.addEventListener("click", () => {
        ...
        if (selectedAction === "trash") {
            openConfirmTrashModal(`${selectedRows.length} validations sélectionnées`, "/admin/validations/corbeille");
            return;
        }
