@php
    ...
    $isEdit = $mission->exists;
    $formAction = $isEdit ? route('admin.missions.update', $mission) : route('admin.missions.store');
    $formMethod = $isEdit ? 'patch' : 'post';
    $submitLabel = $isEdit ? 'Enregistrer les modifications' : 'Enregistrer';
    ...
    $previewImage = $isEdit ? $mission->imageUrl() : asset('assets/img/card-mission/type.png');
    $imageUrlValue = old('image_url', $missionImageMode === 'url' ? $mission->image_path : '');
    $visibleErrors = $errors->any() ? collect($errors->all())->unique()->take(6) : collect();
@endphp

<form class="admin-mission-form" id="mission-create-form" action="{{ $formAction }}" method="post" enctype="multipart/form-data" data-real-form data-admin-mission-form data-category-badges='@json($missionCategoryBadges)'>
    @csrf
    @if($formMethod !== 'post')
        @method($formMethod)
    @endif
            ...
            <div class="admin-image-main">
                ...
                <div class="admin-image-source" data-image-source="url" hidden>
                    <label class="admin-field" for="m-image-url">
                        ...
                        <input id="m-image-url" name="image_url" type="url" value="{{ $imageUrlValue }}" placeholder="https://..." data-image-url disabled>
                    </label>
                </div>
            </div>

            <aside class="admin-image-preview" aria-label="Aperçu de l'image">
                <img src="{{ $previewImage }}" alt="" data-image-preview>
