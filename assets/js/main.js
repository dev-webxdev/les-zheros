...
document.querySelectorAll("[data-word-mystery-tabs]").forEach((tabs) => {
    const root = tabs.closest(".word-mystery-game");
    const tabLinks = Array.from(tabs.querySelectorAll("[data-word-mystery-tab]"));
    const panels = Array.from(root?.querySelectorAll("[data-word-mystery-panel]") || []);
    const rewardPanels = Array.from(root?.querySelectorAll("[data-word-mystery-rewards]") || []);

    if (!root || !tabLinks.length || !panels.length) {
        return;
    }
    ...
    const showDifficulty = (difficulty, url = null) => {
        ...
        panels.forEach((panel) => {
            panel.hidden = panel.dataset.wordMysteryPanel !== difficulty;
        });
    ...
    root.querySelectorAll("[data-word-mystery-form]").forEach((form) => {
